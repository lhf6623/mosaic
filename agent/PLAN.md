...
| C5 | 只有一个运行时依赖 ofa.js，且 pin 在验证过的版本 | 见 [D1](#d1-分发走-gh仓库即产物) 与 [P23](./ofa-pitfalls.md) |
...
│   ├── components.js              # 组件登记表：二级菜单 / 总览 / 首页卡片都由它驱动
...
- [ ] `mc-button`（已实现）/ `mc-icon` / `mc-card` / `mc-badge` / `mc-spinner`
