...
下表是标度参考；精选子集里实际存在的变体只有 `md:` / `lg:`（见规则三）：
...
**规则一：媒体查询在 shadow DOM 里有效，`md:` / `lg:` 可以放心用。**

|                 | 生成的 CSS                                                             | 在 shadow root 内                                              |
| --------------- | ---------------------------------------------------------------------- | -------------------------------------------------------------- |
| `dark:bg-black` | `@media (prefers-color-scheme: dark){ .dark\:bg-black }`——**媒体查询** | ✅ 生效，但只跟系统偏好，跟不上 `<html data-theme>` 的三态切换 |
| `md:flex`       | `@media (min-width:768px){ .md\:flex }`——**媒体查询**                  | ✅ 媒体查询不依赖祖先，正常工作                                |

所以组件内部**可以**用 `@media` 和 `md:` / `lg:` 变体写响应式（但见规则二）；
**`dark:` 一律禁用**——它不失效，而是跟不上站点主题开关，暗色差异一律表达为令牌。

**规则二（目标方向，尚未落地）：组件内部的响应式应该用容器查询（`@container`），而不是媒体查询。**
...
**尚未落地**：精选子集没有容器查询变体，现有组件也还没写过任何响应式断点（连 `@media` 都没有）；
将来新增响应式组件时按本节写。

**规则三：精选工具类子集里的响应式变体是有限的。**
当前只包含 `md:` / `lg:` 下的 `block` / `hidden` / `flex` / `grid-cols-2..4` / `flex-row` / `w-auto`，
共 16 个。其余组合（`sm:mt-4`、`xl:grid-cols-6`…）**不存在**，需要时自己写 CSS。

理由：`断点 × 工具类` 是组合爆炸，而实际布局里 90% 的响应式需求
