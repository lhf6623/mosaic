...
export default async function run({
  ...
  check(
    ...
    codeState.rows
      .map((r) => `${r.lang || '-'}:${r.tokens}${r.balanced ? '' : '(未配平)'}`)
      .join(' '),
  );
  ...
  const codeSources = await codePage.evaluate(async () => {
    ...
    const byAttr = window
      .__deepAll('mc-code')
      .find((el) => (el.getAttribute('code') || '').includes('byAttr'));
  ...
  const wheelChain = await (async () => {
    /*
     * 只看**演示里**的代码块：抽屉里那块 mc-code 住在收起的折叠面板里（尺寸 0），
     * 拿它当滚轮落点会永远滚不动页面。判据是「不在 .doc-demo-code 里」。
     */
    const snapshot = () =>
      codePage.evaluate(() => {
  ...
  const readKeywordColor = () =>
    codePage.evaluate(() => {
      const el = window
        .__deepAll('mc-code')
        .find((c) => c.getAttribute('language') === 'javascript');
  ...
  const codeRuntime = await codePage.evaluate(async () => {
    ...
    const before = el.shadowRoot
      .querySelector('.mc-block')
      .querySelectorAll('span[class*="hljs-"]').length;
    el.setAttribute('language', 'css');
    await new Promise((r) => setTimeout(r, 500)); // P4：setAttribute 异步生效
    const after = {
      tokens: el.shadowRoot.querySelector('.mc-block').querySelectorAll('span[class*="hljs-"]')
        .length,
    ...
    return {
      ...
      sourceLines,
      lines,
      back: el.shadowRoot.querySelectorAll('.mc-line').length,
    };
  });
  check(
  ...
  check(
    ...
    codeRuntime.lines === codeRuntime.sourceLines &&
      codeRuntime.lines > 1 &&
      codeRuntime.back === 0,
  ...
  /* 迁移完成：文档站的代码块全部由 mc-code 渲染，`.doc-code` 样式已退场。
   这一条走主 page（不是上面那个 codePage），所以先把它带到站点入口 —— 单飞跑
   这个套件时主 page 还停在 about:blank，`__deepAll` 是后面 goto 才注入的。 */
  ...
  check(
    ...
    !!srcSnippet.text && srcSnippet.text === srcSnippet.expected && srcSnippet.tokens > 0,
  ...
  check(
    ...
    degradedCode.state.text.length > 0 &&
      degradedCode.state.tokens === 0 &&
      !degradedCode.state.themed,
