...
/* ------------------------------------------------------------------ *
 * 11.4 mc-collapse —— 折叠面板（开合是原生 <details> 的，这里只测组件契约：open 三条路一致 / toggle 反射 / accordion 互斥 / disabled 三件套 / 尺寸靠 CSS 变量穿透子项 shadow root）
 * ------------------------------------------------------------------ */
...
const collapse = await (async () => {
  ...
  const initial = await page.evaluate(() => {
    ...
    const items = [
      ...window.__deepAll('mc-collapse-item', window.__deepAll('demo-collapse-basic')[0].shadowRoot),
    ];
    return {
  ...
  const sizes = await page.evaluate(() => {
    ...
    return window.__deepAll('mc-collapse', box).map((c) =>
      Math.round(
        c
  ...
  const accordion = await page.evaluate(async () => {
    const c = window.__deepAll('mc-collapse', window.__deepAll('demo-collapse-accordion')[0].shadowRoot)[0];
  ...
  const keyboard = await (async () => {
    ...
    const focused = await page.evaluate(() => {
      const item = window.__deepAll('mc-collapse-item', window.__deepAll('demo-collapse-basic')[0].shadowRoot)[0];
    ...
    const afterSpace = await page.evaluate(() => {
      const item = window.__deepAll('mc-collapse-item', window.__deepAll('demo-collapse-basic')[0].shadowRoot)[0];
      return { open: item.open, stillFocused: item.shadowRoot.activeElement?.tagName === 'SUMMARY' };
    });
...
check(
  ...
  collapse.initial.upgraded &&
    collapse.initial.total >= 10 &&
    collapse.initial.collapses >= 5,
