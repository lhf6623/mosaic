...
const colocated = await (async () => {
  ...
  for (const c of READY) {
    ...
    // 带一个 query 也是必须的：从第二个组件开始，`#/…` 只是同文档 hash 跳转，
    // playwright 的 goto() 对同文档导航返回 null（不是 Response），断言会假失败。
    // 加 query 之后每次都走真正的冷启动加载 —— 顺带把路由冷启动也测了。
 ...
 *
 * 三条线：
 *   · 高亮路径：跨行标签配平、行号、soft-wrap、max-height、运行时改属性
 *   · 配色路径：token 颜色必须等于 highlight.js 官方主题样式表里声明的值
 *     （组件不加 hljs 类、也不写覆盖它的规则），切亮暗会换成另一张官方表
 *   · 降级路径：用 route 掐掉高亮库 → 纯文本 + 一条 [mosaic] 警告
 *
 * 另外守两个踩过的 ofa.js 保留名 / 构造期坑（见 P7 / P31）：
 *   · proto 方法不能叫 refresh / sync
 *   · attrs 的键不能叫 wrap；也不能在 ready() 里往宿主写属性
 * 两者都会让 document.createElement('mc-code') 静默坏掉。
 * ------------------------------------------------------------------ */
...
await codePage
  .waitForFunction(
    () => {
      const deepAll = (sel, root = document, acc = []) => {
      ...
      return deepAll('mc-code').length >= 10;
    },
    { timeout: 15000 },
  )
  .catch(() => {});
...
const codeState = await codePage.evaluate(() => {
  ...
  const rows = deepAll('mc-code').map((el) => {
...
const codeSources = await codePage.evaluate(async () => {
  ...
  const byAttr = deepAll('mc-code').find((el) => (el.getAttribute('code') || '').includes('byAttr'));
  const bound = deepAll('mc-code').find((el) => textOf(el).includes('mc-code :code'));
  ...
  const target = deepAll('mc-code')[0];
 ...
 *
 * 两个坑都会让"鼠标停在代码上页面就不动"：
 *   ① `.mc-body` 上写 overscroll-behavior: contain —— 它连"内容不超高、没得滚"
 *      的块都会掐断滚动链，所有代码块都变成滚动陷阱；
 *   ② 限高块在浏览器 scroll latching 下，块内滚到底后**同一次手势里后面几次
 *      滚轮全被吞**（实测第 3 次不动、第 4 次才动）。
...
const wheelChain = await (async () => {
  const snapshot = () =>
    codePage.evaluate(() => {
      ...
      // 注意：ofa.js 会把声明过的字符串属性以**空值**写到宿主上
      // （每个 mc-code 都有 hljs-base="" / max-height=""），
      // 所以这里必须看属性值，不能只看 hasAttribute。
      const limited = deepAll('mc-code').findIndex(
        (el) => (el.getAttribute('max-height') || '').trim() !== '',
      );
      const inner = deepAll('mc-code')[limited].shadowRoot.querySelector('.mc-body');
      const outer = deepAll('.doc-body')[0];
      return {
        limited,
  ...
  const resetOuter = () =>
    codePage.evaluate(() => {
      ...
      deepAll('.doc-body')[0].scrollTop = 0;
    });
  ...
  await codePage.evaluate((i) => {
    ...
    const body = deepAll('mc-code')[i].shadowRoot.querySelector('.mc-body');
...
const readKeywordColor = () =>
  codePage.evaluate(() => {
    ...
    const el = deepAll('mc-code').find((c) => c.getAttribute('language') === 'javascript');
    const kw = el?.shadowRoot.querySelector('.hljs-keyword');
    return kw ? getComputedStyle(kw).color : null;
  });

/*
 * 配色必须"等于 highlight.js 主题里声明的值"，而不是组件自己另写一套：
 * 直接在同一份 adopted 表里找 .hljs-keyword 规则，把它声明的颜色和计算值比。
 */
...
const darkScheme = await codePage.evaluate(() => {
  ...
  return getComputedStyle(deepAll('mc-code')[0]).colorScheme;
});
...
const codeRuntime = await codePage.evaluate(async () => {
  ...
  const el = deepAll('mc-code').find((c) => c.getAttribute('language') === 'html');
...
/*
 * P31 回归：动态 createElement 出来的实例也必须能初始化。
 * 两个坑都会让这一步静默坏掉：attrs 里出现 ofa.js 保留名（wrap），
 * 或者 ready() 里往宿主元素写属性（自定义元素构造期不允许带属性）。
 */
...
/*
 * 降级：高亮库不可达时，"没有颜色"而不是"没有代码"。
 * 单独起一个浏览器实例 —— 同一进程里的 HTTP 缓存是共享的，否则掐不掉。
 */
