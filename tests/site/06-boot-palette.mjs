...
export default async function run({ page, visit, goHash, pageState, check, BASE, newPage }) {
  ...
  check(
    '色板渲染出 66 个色块（6 色族 × 11 档）',
    tokensState.paletteRows === 6,
    `6 个色族 / ${tokensState.paletteRows} 行`,
  );
  ...
  const layoutIdentity = await (async () => {
    ...
    return page.evaluate(() => {
      ...
      return {
        ...
        active:
          layout?.shadowRoot?.querySelector('.doc-top-nav a[aria-current]')?.textContent?.trim() ??
          null,
      };
    });
  })();
