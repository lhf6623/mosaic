...
export default async function run({ page, goTop, goHash, check, browser, BASE }) {
  ...
  const scrolledBefore = await page.evaluate(() =>
    Math.round(window.__deep('.doc-main').scrollTop),
  );
    ...
    page.evaluate(() => {
      ...
      return {
        ...
        layers: [...art.querySelectorAll('[data-layer]')].map(
          (l) =>
            `${Math.round(l.getBoundingClientRect().width)}x${Math.round(l.getBoundingClientRect().height)}`,
        ),
  ...
  check(
    ...
    resized.every(
      (r) => !r.overflow && r.layers.length === 3 && r.layers.every((b) => b === r.box),
    ),
    resized
      .map((r) => `${r.w}×${r.h} → 容器 ${r.box} 三层 ${r.layers.join('/')} ${r.count}块`)
      .join('\n        '),
  );
  ...
  const reduceTransforms = await reducePage.evaluate(() => {
    const art = window.__deep('.poster-art');
    return art
      ? [...art.querySelectorAll('[data-layer]')].map((g) => g.style.transform || '')
      : null;
  });
  check(
