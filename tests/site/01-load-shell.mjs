...
export default async function run({ page, visit, check, problems }) {
  ...
  await page
    .waitForFunction(() => !!window.__deep?.('mc-button'), { timeout: 15000 })
    .catch(() => {});
  ...
  const layerOrder = await page.evaluate(() => {
    ...
    const sr = host.attachShadow({ mode: 'open' });
    sr.innerHTML =
      '<style>.probe { color: rgb(1, 2, 3); }</style><span class="probe text-muted">x</span>';
  ...
  check(
    '文档级令牌也切到了暗色',
    docSurface !== '255 255 255',
    `--mc-color-surface = ${docSurface}`,
  );
