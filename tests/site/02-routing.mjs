...
export default async function run({ page, visit, goTop, goHash, pageState, check }) {
  ...
  // 首页是海报，等它渲染出来再往下断言
  await page
    .waitForFunction(() => !!window.__deep?.('.doc-poster'), { timeout: 15000 })
    .catch(() => {});
  await page.evaluate(() => {
  ...
  check(
    '首页没有二级菜单',
    homeState.navLinks === 0 && homeState.page === '首页',
    `菜单项=${homeState.navLinks} 高亮=${homeState.page}`,
  );
  ...
  check(
    ...
    reloaded.length === 0,
    reloaded.length
      ? `这些页面发生了整页刷新：${reloaded.map((v) => v.route).join(', ')}`
      : `连续切换 ${visits.length} 个页面，标记一直活着`,
  );
  ...
  check(
    `组件总览渲染出全部 ${ALL.length} 张卡片`,
    compState.cards === ALL.length,
    `实际 ${compState.cards} 张`,
  );
  check(
    '组件页自带二级菜单：总览 + 全部组件',
