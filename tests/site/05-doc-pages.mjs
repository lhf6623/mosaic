...
export default async function run({ page, visit, check, newPage }) {
  ...
  const colocated = await (async () => {
    ...
    for (const c of READY) {
      ...
      if (!res?.ok() || failed.length || !navOk) {
        bad.push(
          `packages/${c.slug}/page.html — ${res?.status()}${failed.length ? ' · ' + failed.join(', ') : ''}${navOk ? '' : ' · 二级菜单未渲染'}`,
        );
      }
    }
    await p.close();
  ...
  /* ------------------------------------------------------------------ *
   * 11.3 演示区：例子是独立文件，抽屉就是作者写的一行 mc-collapse
   *
   * 每个 `.doc-demo` 里现在有两样东西：
   *   · `<demo-<组件>-<名字>>` —— 例子本体，一个 ofa 组件文件（demos/*.html，可单独打开/单测）
   *   · `<mc-collapse class="doc-demo-code">` + `<mc-code src="./demos/….html">` —— 查看代码
   *
   * 所以这里盯三条：例子确实来自 demos/ 里的组件文件；代码面板引用的就是**同一个文件**
   * （渲染出来的文本 == 文件原文，不抄第二份）；每个演示都能点开看。
   * ------------------------------------------------------------------ */
  ...
  const demoDrawer = await (async () => {
    ...
    /** 读一页的演示区：例子组件（自己一个 shadow root）+ 抽屉里那块 mc-code */
    const readDemos = () =>
      page.evaluate(() =>
  ...
  check(
    ...
    drawerPages.every(
      (x) =>
        x.demos.length > 0 && x.demos.every((d) => d.demoTag && d.demoShadow && d.drawerExists),
    ),
  ...
  check(
    ...
    drawerPages.every((x) => x.demos.every((d) => d.drawerExists && d.label === '查看代码')),
  ...
  check(
    ...
    drawerPages.every(
      (x) => x.fileMatch.length === x.demos.length && x.fileMatch.every((f) => f.same),
    ),
    JSON.stringify(
      drawerPages
